...
{
	std::string msg = "";
	char tempmsg[MAX_BUFFER_SIZE] = "";

	//Session
	while (1)
	{
		memset(tempmsg, 0, MAX_BUFFER_SIZE);

		if (new_client.socket != 0)
		{
			int iResult = recv(new_client.socket, tempmsg, MAX_BUFFER_SIZE, 0);
...
int main()
{
	...
	std::string msg = "";
	std::vector<client_type> client(MAX_SLOTS);
	int num_clients = 0;
	int temp_id = -1;
	std::thread my_thread[MAX_SLOTS];
	...
	getaddrinfo(static_cast<const char*>(IP_ADDRESS), DEFAULT_PORT, &hints, &server);
	...
	while (1)
	{
		...
		if (temp_id != -1)
		{
			...
			std::cout << client[temp_id].name << " Accepted" << std::endl;
